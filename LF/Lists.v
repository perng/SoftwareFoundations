...
Fixpoint alternate1 (l1 l2 : natlist) : natlist :=
  match l1 with
  | nil => l2
  | h1 :: t1 => match l2 with 
                | nil => l1
                | h2 :: t2 => (h1 :: h2 :: (alternate1 t1 t2))
                end
  end.
...
Theorem add_inc_count : forall  (v:nat) (s : bag), (count v s) + 1 = (count v (v::s)).
Proof.
  intros.
  simpl. 
  assert (H: ((v =? v) = true)).
  {  induction v. simpl. reflexivity. simpl. assumption. }
  rewrite -> H. rewrite -> add_comm. apply plus_1_l.
Qed.
...
Proof.
  intros.
  assert (H234: l2 ++ (l3 ++ l4) = (l2 ++ l3) ++ l4).
  {   rewrite -> app_assoc.  reflexivity. }
  rewrite -> H234.
  rewrite <- app_assoc.
  assert (H123: l1 ++ l2 ++ l3 = (l1 ++ l2) ++ l3).
  {   rewrite -> app_assoc.  
      reflexivity. }
  rewrite <- H123.
  reflexivity.
